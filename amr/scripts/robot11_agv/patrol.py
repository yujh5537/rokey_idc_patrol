...
def build_route(config):
    ...
    if config['robot_zones']['robot11'] != ['Z1', 'Z2']:
        raise ValueError(
            "racks.yaml must assign robot11 to ['Z1', 'Z2']"
        )

    if ids != EXPECTED_ROBOT11_ROUTE:
        raise ValueError(
            'robot11 patrol route does not match the approved Z1/Z2 order'
        )
    ...
    # Undock near robot11 dock
    # -> midpoint between robot11/robot5 docks
    # -> midpoint between Z2/Z3
    # -> Z1
    # -> Z2
    route = [
        (
    ...
    for zone in ('Z1', 'Z2'):
        ...
        zone_ids = [
            rid
            for rid in ids
            if racks[rid]['zone_id'] == zone
        ]

        if len(zone_ids) != 14:
            raise ValueError(
                f'Expected 14 racks in {zone}'
            )

        route.extend(
            pose(
                rid,
                racks[rid]['inspect_pose']
            )
            for rid in zone_ids
        )
    ...
    # Return through the same major waypoints.
    # The exact undock pose is appended at runtime
    # before Create 3 docking.
    route.extend([
        (
            'return_z2_z3_midpoint',
            1.400,
            2.800,
            math.pi
        ),
        (
            'return_dock_midpoint',
            0.270,
            2.625,
            math.pi / 2
        ),
...
def run_patrol(
    ...
    for waypoint in route:
        ...
        if (
            waypoint[0].startswith('R')
            and not inspect(waypoint[0])
        ):
            return False
...
def main():
    ...
    parser.add_argument(
        '--execute',
        action='store_true',
        help='send physical motion goals'
    )
    ...
    print(
        'return_start: capture undocked starting pose '
        'at execution; then /robot11/dock'
    )
    ...
    if not args.execute:
        ...
        print(
            'Preview only. '
            'Use --execute to undock and patrol; --execute --undock-only for staged setup.'
        )
    ...
    lock = open(
        f'/tmp/idc_robot11_route_{os.getuid()}.lock',
        'w'
    )
    ...
    try:
        ...
        print(
            'Another route process is already running on this host.'
        )
    ...
    def cancel():
        ...
        node.get_logger().warning(
            ...
            'no subsequent goal will be sent.'
        )
    ...
    def perform(
        ...
        if (
            ...
            node.get_logger().error(
                f'Goal failed/cancelled: '
                f'status={status}, result={result.result().result}. '
                'Route stopped.'
            )
        ...
        return (
            not require_docked
            or bool(
                result.result().result.is_docked
            )
        )
    ...
    def send_navigation(waypoint):
        ...
        # Navigation success advances the stage; no extra exact pose rejection.
        success = perform(
            client,
            goal,
            args.goal_timeout
    ...
    def turn(label, target_yaw):
        ...
        node.get_logger().info(f'{label}: target={math.degrees(target_yaw):.1f}deg, turn={math.degrees(delta):.1f}deg')
    ...
    def navigate(waypoint):
        if stopped:
            return False
        if waypoint[0] not in EXPECTED_ROBOT11_ROUTE:
    ...
    def inspect(rack_id):
        ...
        node.get_logger().info(
            f'DWELL {rack_id} (no camera/YOLO): '
            f'waiting {args.dwell}s'
        )
    ...
    def dock():
        ...
        node.get_logger().info(
            'DOCK: handing control '
            'to Create 3 docking action'
        )
    ...
    def wait_ready():
        ...
        pending_nodes = set(REQUIRED_NODES)
        last_report = 0.0
        while rclpy.ok() and not stopped and time.monotonic() < deadline:
            pending_nodes = set()
    ...
    try:
        ...
        if not map_matches(map_msg):
            ...
            node.get_logger().error(
                ...
                f"{EXPECTED_MAP['width']}x"
                f"{EXPECTED_MAP['height']} "
                f"origin=("
                f"{EXPECTED_MAP['origin_x']}, "
                f"{EXPECTED_MAP['origin_y']}), "
                f'got res={info.resolution} '
                f'{info.width}x{info.height} '
                f'origin=('
                f'{info.origin.position.x}, '
                f'{info.origin.position.y}). '
                'No navigation goal sent.'
            )
        ...
        node.get_logger().info(
            'MAP-02 metadata verified.'
        )

        # ---------------------------------------------------------
        # 2. Capture undocked robot11 starting pose
        # ---------------------------------------------------------
        ...
        if not dock_client.wait_for_server(
            timeout_sec=5.0
        ):

            node.get_logger().error(
                'Dock action unavailable. '
                'No navigation goal sent.'
            )
